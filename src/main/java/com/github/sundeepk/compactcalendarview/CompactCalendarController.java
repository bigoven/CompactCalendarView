...
class CompactCalendarController {
    ...
    private void init(Context context) {
        ...
        this.todayCalendar = Calendar.getInstance(locale);
    ...
    void setCurrentDate(Date dateTimeMonth) {
        ...
        currentCalendar.setTime(currentDate);
        todayCalendar = Calendar.getInstance(locale);
    ...
    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalendar, int offset) {
        List<Events> uniqEvents = eventsContainer.getEventsForMonthAndYear(currentMonthToDrawCalendar.get(Calendar.MONTH), currentMonthToDrawCalendar.get(Calendar.YEAR));
        ...
        if (uniqEvents != null) {
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                boolean isCurrentSelectedDay = currentCalendar.get(Calendar.DAY_OF_MONTH) == dayOfMonth;
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalendar, int offset) {
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                } else if (currentCalendar.get(Calendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
